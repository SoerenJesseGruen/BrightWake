	...
	var wakeInterval = setInterval( function () {
		if(wakeMode!="") {
			...
			if(wakeMode=="Lichtmodus") {
				setTimeout( function() {
					var secondMode = "Vibrationsmodus"; // oder "SoundModus"
...
function waitForNextWakeEvent_Demo (nextWakeTime) {
	...
	// 1. Prüfzeitpunkt => Tiefschlaf-Weckmodus wird gestartet, wenn 1 Minute vor dem Wecken die letzte Bewegung vor mehr als 1 Minute war
...
function waitForNextWakeEvent_DB (nextWakeTime) {
	...
	var mode = "";
	var currentTime = new Date();
	var lastGyroXMinutesAgo = currentTime.getMinutes()-lastGyroDetection.getMinutes();
	
	console.log("Waiting for " + nextWakeTime + "\tCurrent Time: " + currentTime + "\tlastGyro: " + lastGyroXMinutesAgo);
	...
	if(((nextWakeTime.getMinutes()-currentTime.getMinutes())==30 || (nextWakeTime.getMinutes()-currentTime.getMinutes())==(-30)) && (lastGyroXMinutesAgo>=10 || lastGyroXMinutesAgo<=-10)) {
		var query = "SELECT Weckmodus_Bezeichnung FROM Weckmodus wm INNER JOIN NutzerWeckmodus nwm ON wm.Weckmodus_Id=nwm.FK_Weck_Id INNER JOIN Schlafmodus sm " 
	...
	if(currentTime.getHours()==nextWakeTime.getHours() && currentTime.getMinutes()==nextWakeTime.getMinutes()) {
		// letzte Bewegung vor mehr als 5 Minuten vor dem Wecken wahrgenommen => Nutzer in Normalschlaf
